...
module.exports = {
    ...
    getKeg: function(kegId, connection, outputFunc){
        ...
        request = new Request(kegCheck, function(err, rowCount, rows){
            if(err){
                return outputFunc({code: 500, msg:'Internal Error: '+err});
            }
            else if(rowCount==0){
            ...
            if(resp.code==200)
            {
                this.putKegFinished(tapId, connection, function(out){
                    if(out.code==200)
                    {
                        ...
                        request = new Request(insertKeg, function(err, rowCount, rows){
                            if(err){
                                return outputFunc({code: 500, msg:'Internal Error: '+err});
                            }
                            else if(rowCount==0){
    ...
    putKegFinished: function(tapId, connection, outputFunc){
        ...
        request = new Request(markKegAsFinished, function(err, rowCount, rows){
            if(err){
                    return outputFunc({code: 500, msg:'Internal Error: '+err});
                }
                else{
