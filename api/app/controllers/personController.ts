...
export function getPersonByCardId(cardId: number, connection: tedious.Connection, output: (resp:any) => express.Response) {
    ...
    var request = new tedious.Request(sqlStatement, async (err, rowCount, rows) => {
        if (err) {
            ...
            let validUser = await groupMembership.isUserMember(`${rows[0].EmailName.value}@${process.env.Tenant}`);
            output({code: 200, msg: {
